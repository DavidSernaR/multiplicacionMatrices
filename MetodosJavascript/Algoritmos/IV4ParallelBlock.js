 ...
 */
export async function IV4_parallel_block(matrizA, matrizB, matrizResultado, tamanioBloque) {
  ...
  parentPort.once('message', (workerData) => {
          ...
          for (var i = i1; i < i1 + tamanioBloque && i < matrizResultado.length; i++) {
            for (var j = j1; j < j1 + tamanioBloque && j < matrizResultado.length; j++) {
              for (var k = k1; k < k1 + tamanioBloque && k < matrizResultado.length; k++) {
                // Multiplicación de matrices y actualización de la matriz resultado
    ...
    // Envía el bloque actualizado de A de vuelta al hilo principal
  ...
  try {
    ...
    return matrizResultado; //MatrizResultado se retorna
  } catch (error) {
