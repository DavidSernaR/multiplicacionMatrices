...
from NaivOnArray import naiv_on_array
...
for i in range(1, 9):
    ...
    # Medición del tiempo de ejecución del algoritmo NaivOnArray
    tiempo_inicio = time.time()
    matrizR = naiv_on_array(matrizA, matrizB, matrizResultado, len(matrizA), len(matrizB), len(matrizResultado))# Ejecución del algoritmo
    ...
    almacenar_tiempo_ejecucion_json(
        f"{ruta_carpeta}/TiempoEjecucionNaivOnArrayPy.json",
        f"A{i}XB{i} Py",
        tiempo_finalizacion
    )
    ...
    almacenar_resultados_json(
        f"{ruta_carpeta}/ResultadoNaivOnArrayPy.json",
        f"Matriz A{i}xB{i} con método NaivOnArray en Python",
        matrizR
    )
