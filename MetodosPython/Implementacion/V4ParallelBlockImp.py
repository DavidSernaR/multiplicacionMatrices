...
from V4ParallelBlock import  V4parallel_block
...
for i in range(1, 9):
    ...
    # Medición del tiempo de ejecución del algoritmo V4ParallelBlock
    tiempo_inicio = time.time()
    matrizR = V4parallel_block(matrizA, matrizB, matrizResultado, len(matrizA)//2) # Ejecución del algoritmo
    ...
    almacenar_tiempo_ejecucion_json(
        f"{ruta_carpeta}/TiempoEjecucionV4ParallelBlockPy.json",
        f"A{i}XB{i} Py",
        tiempo_finalizacion
    )
    ...
    almacenar_resultados_json(
        f"{ruta_carpeta}/ResultadoV4ParallelBlockPy.json",
        f"Matriz A{i}xB{i} con método V4ParallelBlock en Python",
        matrizR
    )
