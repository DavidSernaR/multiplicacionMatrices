...
from III5EnhancedParallelBlock import III5enhanced_parallel_block
...
for i in range(1, 4):
    ...
    # Medición del tiempo de ejecución del algoritmo III5EnhancedParallelBlock
    tiempo_inicio = time.time()
    matrizR = III5enhanced_parallel_block(matrizA, matrizB, matrizResultado, len(matrizA) // 2)
    ...
    almacenar_tiempo_ejecucion_json(
        f"{ruta_carpeta}/TiempoEjecucionIII5EnhancedParallelBlockPy.json",
        f"A{i}XB{i} Py",
        tiempo_finalizacion
    )
    ...
    almacenar_resultados_json(
        f"{ruta_carpeta}/ResultadoIII5EnhancedParallelBlockPy.json",
        f"Matriz A{i}xB{i} con método III5EnhancedParallelBlock en Python",
        matrizR
    )
