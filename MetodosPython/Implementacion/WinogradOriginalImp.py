...
for i in range(1, 4):
    ...
    # Medición del tiempo de ejecución del algoritmo WinogradOriginal
    tiempo_inicio = time.time()
    matrizR =   winograd_original(matrizA, matrizB, matrizResultado, len(matrizA), len(matrizB), len(matrizResultado))# Ejecución del algoritmo
    ...
    almacenar_tiempo_ejecucion_json(
        f"{ruta_carpeta}/TiempoEjecucionWinogradOriginalPy.json",
        f"A{i}XB{i} Py",
        tiempo_finalizacion
    )
    ...
    almacenar_resultados_json(
        f"{ruta_carpeta}/ResultadoWinogradOriginalPy.json",
        f"Matriz A{i}xB{i} con método WinogradOriginal en Python",
        matrizR
    )
