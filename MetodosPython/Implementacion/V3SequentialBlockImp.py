...
from V3SequentialBlock import  V3_sequential_block
...
for i in range(1, 4):
    ...
    # Medición del tiempo de ejecución del algoritmo V3SequentialBlock
    tiempo_inicio = time.time()
    matrizR =  V3_sequential_block(matrizA, matrizB, matrizResultado, len(matrizA)//2) # Ejecución del algoritmo
    ...
    almacenar_tiempo_ejecucion_json(
        f"{ruta_carpeta}/TiempoEjecucionV3SequentialBlockPy.json",
        f"A{i}XB{i} Py",
        tiempo_finalizacion
    )
    ...
    almacenar_resultados_json(
        f"{ruta_carpeta}/ResultadoV3SequentialBlockPy.json",
        f"Matriz A{i}xB{i} con método V3SequentialBlock en Python",
        matrizR
    )
