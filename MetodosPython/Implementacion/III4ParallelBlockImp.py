...
from III4ParallelBlock import III4_parallel_block
...
for i in range(1, 9):
    ...
    matrizR = III4_parallel_block(matrizA, matrizB, matrizResultado, len(matrizA) // 2)
    ...
    almacenar_tiempo_ejecucion_json(
        f"{ruta_carpeta}/TiempoEjecucionIII4ParallelBlockPy.json",
        f"A{i}XB{i} Py",
        tiempo_finalizacion
    )
    ...
    almacenar_resultados_json(
        f"{ruta_carpeta}/ResultadoIII4ParallelBlockPy.json",
        f"Matriz A{i}xB{i} con método III4ParallelBlock en Python",
        matrizR
    )
