...
from StrassenNaiv import strassen_naiv
...
for i in range(1, 9):
    ...
    # Medición del tiempo de ejecución del algoritmo StrassenNaiv
    tiempo_inicio = time.time()
    matrizR =  strassen_naiv(matrizA,matrizB,matrizResultado,len(matrizA),len(matrizB),len(matrizResultado))# Ejecución del algoritmo
    ...
    almacenar_tiempo_ejecucion_json(
        f"{ruta_carpeta}/TiempoEjecucionStrassenNaivPy.json",
        f"A{i}XB{i} Py",
        tiempo_finalizacion
    )
    ...
    almacenar_resultados_json(
        f"{ruta_carpeta}/ResultadoStrassenNaivPy.json",
        f"Matriz A{i}xB{i} con método StrassenNaiv en Python",
        matrizR
    )
