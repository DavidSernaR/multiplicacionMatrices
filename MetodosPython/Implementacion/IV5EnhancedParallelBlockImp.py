...
from IV5EnhancedParallelBlock import IV5enhanced_parallel_block
...
for i in range(1, 4):
    ...
    # Medición del tiempo de ejecución del algoritmo IV5EnhancedParallelBlock
    tiempo_inicio = time.time()
    matrizR = IV5enhanced_parallel_block(matrizA, matrizB, matrizResultado, len(matrizA) // 2)
    ...
    almacenar_tiempo_ejecucion_json(
        f"{ruta_carpeta}/TiempoEjecucionIV5EnhancedParallelBlockPy.json",
        f"A{i}XB{i} Py",
        tiempo_finalizacion
    )
    ...
    almacenar_resultados_json(
        f"{ruta_carpeta}/ResultadoIV5EnhancedParallelBlockPy.json",
        f"Matriz A{i}xB{i} con método IV5EnhancedParallelBlock en Python",
        matrizR
    )
