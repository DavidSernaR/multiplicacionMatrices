...
for i in range(1, 4):
    ...
    # Medición del tiempo de ejecución del algoritmo WinogradScaled
    tiempo_inicio = time.time()
    matrizR =   winograd_scaled(matrizA, matrizB, matrizResultado, len(matrizA), len(matrizB), len(matrizResultado))# Ejecución del algoritmo
    ...
    almacenar_tiempo_ejecucion_json(
        f"{ruta_carpeta}/TiempoEjecucionWinogradScaledPy.json",
        f"A{i}XB{i} Py",
        tiempo_finalizacion
    )
    ...
    almacenar_resultados_json(
        f"{ruta_carpeta}/ResultadoWinogradScaledPy.json",
        f"Matriz A{i}xB{i} con método WinogradScaled en Python",
        matrizR
    )
