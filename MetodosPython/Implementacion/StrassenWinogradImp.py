...
from StrassenWinograd import strassen_winograd
...
for i in range(1, 4):
    ...
    # Medición del tiempo de ejecución del algoritmo StrassenWinograd
    tiempo_inicio = time.time()
    matrizR =  strassen_winograd(matrizA,matrizB,matrizResultado,len(matrizA),len(matrizB),len(matrizResultado))# Ejecución del algoritmo
    ...
    almacenar_tiempo_ejecucion_json(
        f"{ruta_carpeta}/TiempoEjecucionStrassenWinogradPy.json",
        f"A{i}XB{i} Py",
        tiempo_finalizacion
    )
    ...
    almacenar_resultados_json(
        f"{ruta_carpeta}/ResultadoStrassenWinogradPy.json",
        f"Matriz A{i}xB{i} con método StrassenWinograd en Python",
        matrizR
    )
