...
from III3SequentialBlock import III3_sequential_block
...
for i in range(1, 9):
    ...
    matrizR = III3_sequential_block(matrizA, matrizB, matrizResultado, len(matrizA) // 2)
    ...
    almacenar_tiempo_ejecucion_json(
        f"{ruta_carpeta}/TiempoEjecucionIII3SequentialBlockPy.json",
        f"A{i}XB{i} Py",
        tiempo_finalizacion
    )
    ...
    almacenar_resultados_json(
        f"{ruta_carpeta}/ResultadoIII3SequentialBlockPy.json",
        f"Matriz A{i}xB{i} con método III3SequentialBlock en Python",
        matrizR
    )
