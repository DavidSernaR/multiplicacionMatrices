...
from IV3SequentialBlock import IV3_sequential_block
...
for i in range(1, 9):
    ...
    # Medición del tiempo de ejecución del algoritmo IV3SequentialBlock
    tiempo_inicio = time.time()
    matrizR = IV3_sequential_block(matrizA, matrizB, matrizResultado, len(matrizA) // 2)
    ...
    almacenar_tiempo_ejecucion_json(
        f"{ruta_carpeta}/TiempoEjecucionIV3SequentialBlockPy.json",
        f"A{i}XB{i} Py",
        tiempo_finalizacion
    )
    ...
    almacenar_resultados_json(
        f"{ruta_carpeta}/ResultadoIV3SequentialBlockPy.json",
        f"Matriz A{i}xB{i} con método IV3SequentialBlock en Python",
        matrizR
    )
