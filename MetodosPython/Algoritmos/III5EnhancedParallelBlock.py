...
"""
    Realiza la multiplicación de matrices por bloques de forma paralela mejorada.
    ...
    """
def III5enhanced_parallel_block(matrizA, matrizB, matrizResultado, tamanioBloque):
        ...
        for i in range(i1, min(i1 + tamanioBloque, tamanio)):
            for j in range(j1, min(j1 + tamanioBloque, tamanio)):
                for k in range(k1, min(k1 + tamanioBloque, tamanio)):
                    resultado[i][j] += matrizA[i][k] * matrizB[k][j]  # Multiplicación y acumula en la posición [i][j]
    ...
    with ThreadPoolExecutor() as executor:
        ...
        for i1 in range(0, tamanio // 2, tamanioBloque):
        ...
        for i1 in range(tamanio // 2, tamanio, tamanioBloque):
            for j1 in range(0, tamanio, tamanioBloque):
                for k1 in range(0, tamanio, tamanioBloque):
                    futures.append(executor.submit(task, i1, j1, k1)) # Envía la tarea al executor y agrega el futuro a la lista
